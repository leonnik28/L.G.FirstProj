...
public class HexFeatureManager : MonoBehaviour
{
    ...
    public void AddWall(EdgeVertices near, HexCell nearCell, EdgeVertices far, HexCell farCell, bool hasRiver, bool hasRoad)
    {
        if (nearCell.Walled != farCell.Walled && !nearCell.IsUnderWater && !farCell.IsUnderWater && nearCell.GetEdgeType(farCell) != HexEdgeType.Cliff)
        {
            ...
            if (hasRoad)
            {
                AddWallSegment(near.v2, far.v2, Vector3.Lerp(near.v2, near.v3, 0.5f), Vector3.Lerp(far.v2, far.v3, 0.5f));
                AddWallSegment(Vector3.Lerp(near.v3, near.v4, 0.5f), Vector3.Lerp(far.v3, far.v4, 0.5f), near.v4, far.v4);

                AddWallCap(Vector3.Lerp(near.v2, near.v3, 0.5f), Vector3.Lerp(far.v2, far.v3, 0.5f));
                AddWallCap(Vector3.Lerp(far.v3, far.v4, 0.5f), Vector3.Lerp(near.v3, near.v4, 0.5f));
            }
            else if (hasRiver)
            {
    ...
    public void AddFeature(HexCell cell, Vector3 position) {
        ...
        if (prefab)
        {
            if (otherPrefab && hash.b < hash.a) prefab = otherPrefab;
        }
        else if (otherPrefab)
        {
            prefab = otherPrefab;
        ...
        if(prefab != otherPrefab) position.y += instance.localScale.y * 0.5f;
